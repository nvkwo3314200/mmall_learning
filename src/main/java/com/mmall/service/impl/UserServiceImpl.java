...
public class UserServiceImpl implements IUserService {
	...
	public ResponseService<User> login(String username, String password) {
		...
		if(count == 0) {
			return ResponseService.createErrorResposeMessage("用户名不存在");
		}
		 //  md5 encrypt password
		String md5Passwrod = MD5Util.MD5EncodeUtf8(password);
		User user = userMapper.checkLogin(username, md5Passwrod);
		if(user == null) return ResponseService.createErrorResposeMessage("用户密码错误");
	...
	public ResponseService<String> register(User user) {
		...
		if(count > 0) {
			return ResponseService.createSuccessResposeMessage("注册成功");
		}
		
		return ResponseService.createErrorResposeMessage("注册失败");
	}

	@Override
	public ResponseService<String> checkValid(String str, String type) {
		if(StringUtils.isBlank(str)) {
			return ResponseService.createErrorResposeMessage("被检验数据不能为空");
		}
		if(StringUtils.isNotBlank(type)) {
			if(Constant.RegisterValidType.USERNAME.equals(type)) {
				int count = userMapper.checkUserExist(str);
				if(count > 0) {
					return ResponseService.createErrorResposeMessage("用户已存在");
				}
			}
			...
			if(Constant.RegisterValidType.EMAIL.equals(type)) {
				...
				if(count > 0) {
					return ResponseService.createErrorResposeMessage("用户邮箱已存在");
				}
			}
		}
		else {
			return ResponseService.createErrorResposeMessage("校验类型不能为空");
		}
		
		return ResponseService.createSuccessResposeMessage("校验成功");
	}
		...
		if(StringUtils.isNotBlank(username)) {
			...
			if(!validResponse.isSuccess()) {
				...
				if(StringUtils.isNotBlank(question)) {
					return ResponseService.createSuccessRespose(question);
				}
				else {
					return ResponseService.createErrorResposeMessage("该用户未设置找回密码问题");
				}
			}
			else {
				return ResponseService.createErrorResposeMessage("用户不存在");
			}
		}
		return ResponseService.createErrorResposeMessage("用户名不能为空");
	}

	@Override
	public ResponseService<String> forgetCheckAnswer(String username, String question, String answer) {
		if(StringUtils.isBlank(username)) return ResponseService.createErrorResposeMessage("用户名不能为空");
		int count = userMapper.checkAnswer(username, question, answer);
		if(count > 0) {
			String token = generateToken();
			TokenCache.setKey(TokenCache.TOKEN_PREFIX + username, token);
			return ResponseService.createErrorRespose(token);
		}
		return ResponseService.createErrorResposeMessage("问题答案错误");
	}
	...
	public ResponseService<String> forgetResetPassword(String username, String passwordNew, String forgetToken) {
		...
		if(StringUtils.equals(token, forgetToken)) {
			...
			if(count > 0) {
				...
				return ResponseService.createSuccessResposeMessage("修改密码成功");
			}
			return ResponseService.createErrorResposeMessage("修改密码操作败");
		}
		return ResponseService.createErrorResposeMessage("token已经失效");
	}
	...
	public ResponseService<String> resetPassword(String passwordOld, String passwordNew, User user) {
		...
		if(count > 0) {
			...
			if(countU > 0) return ResponseService.createSuccessResposeMessage("修改密码成功");
		}
		return ResponseService.createErrorResposeMessage("旧密码输入错误");
	}
	...
	public ResponseService<User> updateInformation(User user, User sessionUser) {
		...
		if(updateCount > 0) {
			...
			if(lastestUser == null) {
				return ResponseService.createErrorResposeMessage("更新成功， 获取用户信息失败");
			}
			lastestUser.setPassword(null);
			return ResponseService.createSuccessRespose(lastestUser);
		}
